...
def mvn_plot_contours(z, label=False, ax=None):
    ...
    """
    # TODO: Make a plotting utils module?
    sess = ed.get_session()
    mu = sess.run(z.mu)
    mu_x, mu_y = mu
...
def retrieve_samples(qz):
    """
    Retrieve the MCMC samples from an Empirical object.
    """
    # TODO: Is there a built-in way to do this in Edward?
    sess = ed.get_session()
    return sess.run(qz.get_variables()[0])
